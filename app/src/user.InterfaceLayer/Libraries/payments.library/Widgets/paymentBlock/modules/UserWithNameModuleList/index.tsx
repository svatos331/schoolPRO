...
const UsersWithNameModuleList: FC<IUserWithNameWidgetProps> = ({getMe, getAllUsers}) => {
    const dispatch = useAppDispatch();

    const {changeSelectedUserId} = paymentsActions;

    // const selectedUserId = useAppSelector(state => state.payments.selectedUserId);
    const {data: users = [],isLoading:isLoadingAll} = getAllUsers({});
    const {data: me= {"name": "", src: ""} as IBaseInfoUserDTO,isLoading: isLoadingMe } = getMe({});
