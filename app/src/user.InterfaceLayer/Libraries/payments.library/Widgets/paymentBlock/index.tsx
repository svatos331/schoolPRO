...
import {
    paymentsActions
} from "../../../../../business.InterfaceLayer/store/shared/entities/payments.entities/payments.entity/redux/slice";


const PaymentBlock: FC<IPaymentBlockProps> = ({putMoney,selectedUserIdSelector, paySumSelector, getMe, getUsers}) => {
    const [putMoneyMutation, {isLoading: isPutMoneyLoading, isError: isPutMoneyError}] = putMoney({});
    const {data:me, isLoading:isBalanceLoading, isError : isGetMeError} = getMe({});
    ...
    const showSuccess = () => {
        toast.current?.show({severity:"success", summary: "Success", detail:"Оплата прошла успешно", life: 3000});
    };
    const showError = () => {
        toast.current?.show({severity:"error", summary: "Error", detail:"Ошибка", life: 3000});
    };
    const onKeyBoardItemClick = useCallback((val:string) => {
            ...
            <ST.Input>
                <label htmlFor="urrency-us">
                    <Typography_Montserrat_Normal
                        ...
                        text={"Enter amount"} color={Colors.BLUE}
                    />
                </label>
                <InputNumber
                    maxFractionDigits={0} min={1} inputId="currency-us"
                    value={sum | 0} onValueChange={(e: InputNumberValueChangeEvent) => setSum(e?.value ?? 0 | 0)}
